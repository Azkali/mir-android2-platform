...
public:
    explicit Display(
        ...
        std::shared_ptr<mir::gl::ProgramFactory> const& gl_program_factory,
...
private:
    ...
    std::shared_ptr<mir::gl::ProgramFactory> const gl_program_factory;
    std::map<DisplayName, std::unique_ptr<DisplaySink>> mutable display_sinks;
    ...
    std::unordered_map<unsigned,Frame> last_frame;
};
